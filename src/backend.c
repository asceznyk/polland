...
int backend_connect(struct backend_state *backend, const char *ip, uint16_t port) {
  ...
  backend->fd = fd;
  printf("backend_connect: fd=%d in_state=%d\n", fd, backend->in_state);
  return 0;
}
...
static void backend_epoll_toggle_write(
  int epfd,
  struct client_state *client,
  bool add_write
) {
  printf("backend_epoll_add_write: add_write = %d\n", add_write);
  struct backend_state *backend = &client->backend;
  struct epoll_event evt = {0};
...
int backend_handle_write(int epfd, struct client_state *client) {
  ...
  for (;;) {
    ...
    if (backend->in_state == BE_REQ_COMPLETE) {
      backend_adv_in_state(backend);
      backend_epoll_toggle_write(epfd, client, 0);
      return 0;
    }
    if (
      backend->in_state == BE_WRITING_HEADERS ||
